...
import { ArrowRight, Check, Zap, AlertTriangle, ChevronRight } from "lucide-react";
...
function ConceptDiagram({ components }: { components: string[] }) {
  const display = components.filter((c) => c !== "client").slice(0, 4);
  return (
    <div className="relative flex items-center justify-center gap-0 py-2">
      {display.map((comp, idx) => {
